...
def fcfs(init_processes):
    ...
    while(len(finished_processes) < total_length):
        ...
        active_process = sf.set_active(active_process, arrived_processes)
        init = len(init_processes)
        arr = len(arrived_processes)
        fin = len(finished_processes)
        print("%d and %d and %d and %r" % (init, arr, fin, active_process != None))
        time_elapsed += 1
        waw = 'waw'
